...
public interface IRespuestaJugadorRepository extends JpaRepository<RespuestaJugador, Integer>{
	...
	@Query(value="SELECT idrespuestajugador, nombrejugador, paralelo, puntaje, COUNT(puntaje) as respuestas , idbancopreguntas FROM respuestajugador WHERE idbancopreguntas=:idbancopreguntas GROUP BY puntaje", nativeQuery = true)
